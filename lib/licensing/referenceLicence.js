...
// and NOT a wrapper around any teracom-ai-backend endpoint. There is no such
// endpoint: teracom-ai-backend has zero billing/licensing support today —
// no `plan`/`tier`/`licence` field anywhere in its schema, no
// organisations.plan column, no /licensing/* router among its 39 registered
// routers. Confirmed by reading every model and grepping the entire backend
// source for "licen|billing|plan|subscription|stripe|seat" (zero matches
// outside this frontend's own docs) — see
// BILLING_AND_LICENSING_IMPLEMENTATION_REPORT.md §2.
//
