...
export const HELP_CONTENT = {
  ...
  // CUSTOMER_UX_ACCEPTANCE_V1 -- "Clarify: Worker, Department Head,
  // Executive Worker, Orchestrator." Every other role in that list
  // already had its own entry above; this was the one gap. Framed the
  // same honest way department/executiveTeam already frame it: there is
  // no standalone Orchestrator account or persona to create.
  orchestrator: {
    title: 'Orchestrator',
    whatIsIt: 'Not a Worker you create or a role you assign — it\'s the name for whichever part of the platform is actively performing Orchestration for one request, most visibly CTO Orchestration.',
    whyItExists: 'Gives you a single, consistent name for "the thing that routed my request", without implying there\'s a dedicated employee behind it that needs its own Knowledge, Memory, or Department.',
    example: 'Submitting a CTO Orchestration request and seeing it produce tasks split across Engineering, Sales, and Documentation — that split is the Orchestrator at work, not a Worker of its own.',
    relatedConcepts: [
      {
        label: 'Orchestration',
        text: 'Orchestrator is the actor; Orchestration is the process it carries out — breaking a request into subtasks and routing each one.',
      },
      {
        label: 'Workers',
        text: 'The Orchestrator routes work to real Workers, but is not a Worker itself — it has no Knowledge, Memory, or Department of its own.',
      },
      {
        label: 'Department Heads',
        text: 'A Department Head coordinates within one Department. The Orchestrator coordinates across every Department for a single request.',
      },
    ],
  },
  // CUSTOMER_UX_ACCEPTANCE_V1 -- "Explain purpose and relationship to
  // Orchestration" for Tasks.
  task: {
    title: 'Tasks',
    whatIsIt: 'A single unit of work, assigned to a Worker (or a human) inside one Project, with a status of pending, in progress, or done.',
    whyItExists: 'Gives a request somewhere concrete to live once it\'s been broken down — the actual unit that gets picked up, worked, and marked complete.',
    example: 'A CTO Orchestration request splits into three Tasks — one in Engineering, one in Sales, one in Documentation — each tracked here independently.',
    relatedConcepts: [
      {
        label: 'Orchestration',
        text: 'Orchestration is how a larger request gets split up; the Tasks it produces are what you see and track on this page.',
      },
      {
        label: 'Projects',
        text: 'Every Task belongs to exactly one Project. This page shows Tasks across every Project at once; a Project\'s own workspace shows only its Tasks.',
      },
    ],
  },
  departmentOverrides: {
