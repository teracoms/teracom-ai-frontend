...
// (focus area 6). "Move platform toward: Home, People, Conversations,
// Projects, Outputs, Protection. Retain administration functionality.
// Do not remove existing functionality." These five entries are added at
// the front of the list, ahead of every pre-existing group -- the
// customer-facing vocabulary now leads visually, while every existing
// link below (Workforce, Onboarding, Governance, Reporting, Platform, My
// Organisation, and every admin-only link) is unchanged and still fully
// reachable, per the explicit "do not remove" instruction. "Dashboard" is
// relabelled "Home" (same href, unchanged) rather than duplicated, since
// the two names describe the exact same page. "Projects" already had a
// nav entry inside Workforce (UX_REVIEW_CUSTOMER_PLATFORM_V1.md §H2); it
// keeps that entry and gains this second, top-level one too -- additive,
// not a move. "Protection" points at the peer AI_ORGANISATION_PROTECTION_
// EXPERIENCE_V1 workstream's own /portal/protection route.
export const NAV_ITEMS = [
  { kind: 'link', href: '/portal/dashboard', label: 'Home' },
  // CUSTOMER_EXPERIENCE_REDESIGN_V3 Sec3 -- "People" and "Conversations"
  // merged into one real surface, /portal/team, per
  // CUSTOMER_EXPERIENCE_REDESIGN_V2 Sec6/Sec12 item 5's own finding
  // that they reached substantially the same content two different
  // ways. /portal/people and /portal/conversations both still exist,
  // as real redirects (nothing deleted), so any existing bookmark or
  // external link still resolves -- they're just no longer their own
  // separate nav entries.
  { kind: 'link', href: '/portal/team', label: 'Executive Team' },
  { kind: 'link', href: '/portal/projects', label: 'Projects' },
  { kind: 'link', href: '/portal/outputs', label: 'Outputs' },
  { kind: 'link', href: '/portal/protection', label: 'Protection' },
  {
    kind: 'group',
    label: 'Workforce',
    links: [
  ...
  // CUSTOMER_EXPERIENCE_REDESIGN_V1 item 5 -- "Workflow Wizard remains
  // for administration only." Relabelled for clarity (this is
  // organisation setup: departments/workers/knowledge/governance, not
  // where a customer starts a piece of work -- that's now
  // /portal/start). adminOnly added: this page's own source
  // (app/portal/(protected)/onboarding-wizard/page.js) already fetches
  // almost everything conditionally on isAdmin, leaving a non-admin
  // visitor a mostly-empty page today -- hiding the nav entry from
  // them removes a broken-looking dead end, not real functionality.
  { kind: 'link', href: '/portal/onboarding-wizard', label: 'Administration Setup', adminOnly: true },
  ...
  },
  {
    ...
    links: [
      ...
      // create an Installed Operating Systems area." Technical Support OS
      // is the one real, working module behind this link today; Software
      // Development OS and Cybersecurity OS are named, real product
      // concepts (per this estate's own Operating System Product
      // Framework) with no live-provisioning mechanism for any customer
      // organisation yet -- the list page itself says so honestly rather
      // than implying all three are equally real.
      { href: '/portal/operating-systems', label: 'Installed Operating Systems' },
    ],
  },
];
...
// CUSTOMER_EXPERIENCE_REDESIGN_V3 Sec1 -- which of NAV_ITEMS' own top-
// level entries PortalNav.js renders depends on the viewer's real,
// persisted navigation-mode preference (services/settings_service.py's
// new "navigation": {"mode": ...} key). Deliberately a *presentation*
// split derived from NAV_ITEMS above, not a second, driftable copy of
// its data -- WorkforceNav.js/MyOrganisationNav.js/PlatformSectionNav.js
// keep reading GROUPS exactly as before, completely unaffected by which
// mode is currently selected, since those are each page's own own
// sub-nav tab strip, not the top bar.
//
// Customer Mode (default for every role, including admin/owner): five
// plain links, no dropdown groups -- Home, Executive Team (the real
// People/Conversations merge, Sec3), Projects, Outputs, Protection.
const CUSTOMER_MODE_HREFS = new Set([
  '/portal/dashboard',
  '/portal/team',
  '/portal/projects',
  '/portal/outputs',
...
export const CUSTOMER_NAV_ITEMS = NAV_ITEMS.filter(
  (item) => item.kind === 'link' && CUSTOMER_MODE_HREFS.has(item.href)
);

// Administration Mode (opt-in, admin-tier and above only -- gated by
// PortalNav.js's own existing isAtLeastRole() check, the identical
// mechanism every adminOnly item already uses): everything else,
// unchanged at its existing route -- a re-grouping of NAV_ITEMS, not a
// rebuild. Not nested under one further "Administration" umbrella item
// -- this list *is* the top bar while this mode is selected.
export const ADMINISTRATION_NAV_ITEMS = NAV_ITEMS.filter(
  (item) => !(item.kind === 'link' && CUSTOMER_MODE_HREFS.has(item.href))
);
