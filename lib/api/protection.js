...
// MARKETPLACE_AND_IP_PROTECTION_V1 -- the consolidated Marketplace
// Protection Report: the generalised pack entitlement audit (across
// Workers/Knowledge/DepartmentMemory/Projects/Tasks, not Workers
// alone), pack maintenance status, and a Knowledge ownership
// breakdown (customer/vendor/marketplace-owned). Admin-only, always
// the caller's own organisation.
export async function fetchMarketplaceProtectionReport(token) {
  return backendFetch('/protection/marketplace-report', { token });
}
