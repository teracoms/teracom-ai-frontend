...
const { fetchMemorySummary, storeMemory, archiveMemory } = await import('../memory.js');
...
test('archiveMemory PATCHes /memory/{id}/archive with worker_id as a query param', async () => {
  let seenUrl;
  let seenMethod;
  mockFetch(async (url, init) => {
    seenUrl = url.toString();
    seenMethod = init.method;
    return new Response(
      JSON.stringify({ id: 'm1', worker_id: 'w1', memory_type: 'fact', memory_content: '...', is_archived: true }),
      { status: 200 }
    );
  });

  const data = await archiveMemory('tok', 'm1', 'w1');

  assert.equal(seenUrl, 'https://backend.test/memory/m1/archive?worker_id=w1');
