...
test('fetchKnowledgeMetadata calls GET /knowledge/{id}/metadata', async () => {
  let seenUrl;
  mockFetch(async (url) => {
    seenUrl = url.toString();
    return new Response(
      JSON.stringify({ document_type: 'policy', sensitivity_level: 'internal', tags: ['onboarding'] }),
      { status: 200 }
    );
  });

  const data = await fetchKnowledgeMetadata('tok', 'k1');

  assert.equal(seenUrl, 'https://backend.test/knowledge/k1/metadata');
  assert.deepEqual(data, { document_type: 'policy', sensitivity_level: 'internal', tags: ['onboarding'] });
});

test('updateKnowledgeMetadata PATCHes the given fields to /knowledge/{id}/metadata', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    return new Response(
      JSON.stringify({ document_type: 'policy', sensitivity_level: null, tags: [] }),
      { status: 200 }
    );
  });

  await updateKnowledgeMetadata('tok', 'k1', { document_type: 'policy', tags: [] });

  assert.equal(seenUrl, 'https://backend.test/knowledge/k1/metadata');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { document_type: 'policy', tags: [] });
});
