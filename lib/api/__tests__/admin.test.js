...
const { fetchUsers, createUser, updateUserRole, updateUserStatus, fetchPermissions, createPermission } =
...
test('updateUserRole PATCHes {role} to /users/{id}/role', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenBody = JSON.parse(init.body);
    return new Response(
      JSON.stringify({ id: 'u1', email: 'a@example.com', role: 'manager' }),
      { status: 200 }
    );
  });

  const data = await updateUserRole('tok', 'u1', 'manager');

  assert.equal(seenUrl, 'https://backend.test/users/u1/role');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { role: 'manager' });
  assert.deepEqual(data, { id: 'u1', email: 'a@example.com', role: 'manager' });
});

test('updateUserStatus PATCHes {status} to /users/{id}/status', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenBody = JSON.parse(init.body);
    return new Response(
      JSON.stringify({ id: 'u1', email: 'a@example.com', status: 'inactive' }),
      { status: 200 }
    );
  });

  const data = await updateUserStatus('tok', 'u1', 'inactive');

  assert.equal(seenUrl, 'https://backend.test/users/u1/status');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { status: 'inactive' });
  assert.deepEqual(data, { id: 'u1', email: 'a@example.com', status: 'inactive' });
});
