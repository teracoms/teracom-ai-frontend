...
test('assignDepartmentHead PATCHes {worker_id} to /departments/{id}/head, including an explicit null', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenMethod = init.method;
    seenBody = JSON.parse(init.body);
    return new Response(JSON.stringify({ id: 'd1', head_worker_id: null }), { status: 200 });
  });

  await assignDepartmentHead('tok', 'd1', null);

  assert.equal(seenUrl, 'https://backend.test/departments/d1/head');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { worker_id: null });
});

test('fetchDepartmentWorkers GETs /departments/{id}/workers', async () => {
  let seenUrl;
  mockFetch(async (url) => {
    seenUrl = url.toString();
    return new Response(JSON.stringify([]), { status: 200 });
  ...
  await fetchDepartmentWorkers('tok', 'd1');

  assert.equal(seenUrl, 'https://backend.test/departments/d1/workers');
});

test('assignDepartmentFunction PATCHes {function} to /departments/{id}/function, including an explicit null', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenMethod = init.method;
    seenBody = JSON.parse(init.body);
    return new Response(JSON.stringify({ id: 'd1', function: null }), { status: 200 });
  });

  await assignDepartmentFunction('tok', 'd1', null);

  assert.equal(seenUrl, 'https://backend.test/departments/d1/function');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { function: null });
});
