...
test('setProposalCostEstimate PATCHes {internal_cost_estimate} to /proposals/{id}/cost-estimate', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenMethod = init.method;
    seenBody = JSON.parse(init.body);
    return new Response(JSON.stringify({ id: 'p1', internal_cost_estimate: 500 }), { status: 200 });
  });

  await setProposalCostEstimate('tok', 'p1', 500);

  assert.equal(seenUrl, 'https://backend.test/proposals/p1/cost-estimate');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { internal_cost_estimate: 500 });
});
