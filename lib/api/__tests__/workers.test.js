...
test('fetchWorkerMemories with includeArchived adds include_archived=true', async () => {
  let seenUrl;
  mockFetch(async (url) => {
    seenUrl = url.toString();
    return new Response(JSON.stringify([]), { status: 200 });
  ...
  await fetchWorkerMemories('tok', 'w1', { includeArchived: true });
  assert.equal(seenUrl, 'https://backend.test/memory/w1?include_archived=true');
});
...
test('updateWorkerStatus PATCHes {status} to /workers/{id}/status', async () => {
  let seenUrl;
  let seenBody;
  mockFetch(async (url, init) => {
    seenUrl = url.toString();
    seenBody = JSON.parse(init.body);
    return new Response(JSON.stringify({ id: 'w1', status: 'inactive' }), { status: 200 });
  });

  await updateWorkerStatus('tok', 'w1', 'inactive');

  assert.equal(seenUrl, 'https://backend.test/workers/w1/status');
  assert.deepEqual(seenBody, { status: 'inactive' });
});

test('assignWorkerExecutiveOwner PATCHes {executive_owner_id} to /workers/{id}/executive-owner, including an explicit null', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenMethod = init.method;
    seenBody = JSON.parse(init.body);
    return new Response(JSON.stringify({ id: 'w1', executive_owner_id: null }), { status: 200 });
  });

  await assignWorkerExecutiveOwner('tok', 'w1', null);

  assert.equal(seenUrl, 'https://backend.test/workers/w1/executive-owner');
  assert.equal(seenMethod, 'PATCH');
  assert.deepEqual(seenBody, { executive_owner_id: null });
});
