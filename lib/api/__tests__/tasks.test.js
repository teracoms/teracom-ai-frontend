...
const { createTask, updateTaskStatus, fetchTasks, executeTask, fetchTaskExecutions } = await import('../tasks.js');
...
test('executeTask POSTs to /tasks/{id}/execute with no body', async () => {
  let seenUrl;
  let seenMethod;
  let seenBody;
  mockFetch(async (url, init) => {
    ...
    seenBody = init.body;
    return new Response(JSON.stringify({ id: 'e1', status: 'succeeded' }), { status: 200 });
  });

  await executeTask('tok', 't1');

  assert.equal(seenUrl, 'https://backend.test/tasks/t1/execute');
  assert.equal(seenMethod, 'POST');
  assert.equal(seenBody, undefined);
});

test('fetchTaskExecutions GETs /tasks/{id}/executions', async () => {
  let seenUrl;
  mockFetch(async (url) => {
    seenUrl = url.toString();
    return new Response(JSON.stringify([]), { status: 200 });
  ...
  await fetchTaskExecutions('tok', 't1');

  assert.equal(seenUrl, 'https://backend.test/tasks/t1/executions');
});
