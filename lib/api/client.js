...
export async function backendFetch(path, options = {}) {
  ...
  // Commerce-to-Licensing Lifecycle Automation, Phase 1 — the Stripe
  // webhook handler has no user session token to send, only a shared
  // service secret (X-Internal-Service-Token); `headers` lets a caller
  // add that without every other caller needing to know about it.
