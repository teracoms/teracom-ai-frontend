...
export async function fetchWorkerMemories(token, workerId, { includeArchived = false } = {}) {
  return backendFetch(`/memory/${workerId}`, {
    token,
    searchParams: includeArchived ? { include_archived: 'true' } : undefined,
  });
}
...
// Phase 0 Package PQR (worker lifecycle management, objective #1) — admin-only.
export async function updateWorkerStatus(token, workerId, status) {
  return backendFetch(`/workers/${workerId}/status`, { method: 'PATCH', token, body: { status } });
}

// CUSTOMER_ONBOARDING_WIZARD_V1.md Step 4 — "Workforce Assignment."
// `executiveOwnerId: null` clears the assignment, matching
// assignWorkerDepartment's own convention (lib/api/departments.js).
export async function assignWorkerExecutiveOwner(token, workerId, executiveOwnerId) {
  return backendFetch(`/workers/${workerId}/executive-owner`, {
    method: 'PATCH',
    token,
    body: { executive_owner_id: executiveOwnerId },
  });
}
