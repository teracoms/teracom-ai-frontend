...
// Phase 0 Package M — an internal cost-to-deliver figure, distinct from
// the customer-facing `amount` above. Not a pricing decision, so no
// admin gate backend-side.
export async function setProposalCostEstimate(token, proposalId, costEstimate) {
  return backendFetch(`/proposals/${proposalId}/cost-estimate`, {
    method: 'PATCH',
    token,
    body: { internal_cost_estimate: costEstimate },
  });
}
