...
// not a hypothetical.
const PROJECT_STATUSES = ['active', 'completed', 'blocked'];
...
export function parseUserSettingsUpdatePayload(payload) {
  ...
  if (payload?.preferences !== undefined) {
    ...
    const allowedTopKeys = ['theme', 'accessibility', 'dashboard', 'notifications'];
...
// optional free text.
const AI_PROVIDERS = ['ollama', 'openai', 'anthropic', 'gemini', 'copilot'];
...
export function parseAIProviderConfigPayload(payload) {
  ...
  return { valid: true, provider, model_name: modelName };
}
