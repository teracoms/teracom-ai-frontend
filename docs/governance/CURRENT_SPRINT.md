...
4. **A real update/delete (or explicit archive) capability**, now overdue across twelve distinct rows instead of one: `WorkerMemory` (Package 6), `DepartmentMemory`/`OrganisationMemory`/`MemorySummary` (Package H), `Proposal`/`Quote`/`Contract`/`OnboardingTask` (Package J), and `Campaign`/`ContentPiece`/`VideoAsset`/`MediaCentreItem` (Package K) all share the identical "create and read/decide only" gap. A single backend conversation about correction/removal semantics could address all of these at once rather than one at a time.
...
6. **Wire `Department.function` (Package J, now also `"marketing"` per Package K) into CTO Orchestration's delegation routing (Package I)** — currently the two don't talk to each other; `function` only identifies a department for dashboard purposes today.
...
The backend schema conversation for billing/licensing (flagged as the standing "most urgent non-code gap" since Package 1) remains the single blocking dependency for turning Package 9 from a UX scaffold into a real product feature — unaffected by Package G/H/I/J/K, which extended the product in an unrelated direction (organisational intelligence, memory, the Department Head layer, a native sales/customer-success CRM, and now a native marketing/media production pipeline).
