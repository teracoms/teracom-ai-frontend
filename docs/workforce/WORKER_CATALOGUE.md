...
**Status:** Roster of 9 worker types approved by the project owner, 2026-08-15. This is the commercial/functional catalogue definition — for the technical mechanism a "worker" runs on, see [[backend-status]] §Domain model. For how workers map to editions/seats, see [[product-editions]].
...
- Can a customer create custom worker types outside this catalogue of 9, or is the catalogue meant to be the exhaustive set the product supports? The backend's `workers` table technically allows any `name`/`role`/`purpose`/`instructions` combination today — whether the product intends to constrain this to the catalogue is a commercial/UX decision, not decided.
